...
public class 알고리즘_수업_퀵_정렬_1 {
    static int[] swap = {-1, -1}; // 교환되는 두 수
        ...
        if (start < end) {
            ...
            for ( ; j < end; j++) {
                if (data[j] <= pivot) {
                    ...
                    data[j] = temp;
                    // swap에 교환되는 두 수 저장
                    swap[0] = (i < j) ? data[i] : data[j];
                    swap[1] = (i > j) ? data[i] : data[j];
            ...
            if (data[i + 1] != pivot) {
                ...
                data[end] = temp;
                // swap에 교환되는 두 수 저장
                swap[0] = (i + 1 < end) ? data[i + 1] : data[end];
                swap[1] = (i + 1 > end) ? data[i + 1] : data[end];
                if (--exchange == 0) return; // 남은 교환 횟수 감소
            }
            int mid = i + 1; // 피벗의 위치
