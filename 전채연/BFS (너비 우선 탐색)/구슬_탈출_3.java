...
public class 구슬_탈출_3 {
    ...
    static class Position {
        ...
        public Position(Coordinate red, Coordinate blue, String spin) {
            this.red = red;
            this.blue = blue;
            this.spin = spin;
        }
    ...
    static String bfs(Position start) {
        ...
        while (!que.isEmpty()) {
            ...
            if (next.spin.length() >= 10) return "-1";
            ...
            Spin : for (int i = 0; i < 4; i++) {
                ...
                if (isRedHole) return next.spin + getDirection(i);

                // 구슬들이 모두 제자리인 경우
                if (next.red.x == rx && next.red.y == ry && next.blue.x == bx && next.blue.y == by) continue;

                visited[rx][ry] = true;
                que.add(new Position(new Coordinate(rx, ry), new Coordinate(bx, by), next.spin + getDirection(i)));
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        String result = bfs(new Position(red, blue, ""));
        // 최소 몇 번 만에 빨간 구슬을 구멍을 통해 빼낼 수 있는지 출력
        if (!result.equals("-1")) System.out.println(result.length());
        // 어떻게 기울여야 하는지 순서대로 출력
