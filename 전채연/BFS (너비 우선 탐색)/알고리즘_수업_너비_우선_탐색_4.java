...
public class 알고리즘_수업_너비_우선_탐색_4 {
    static ArrayList<ArrayList<Integer>> graph = new ArrayList<ArrayList<Integer>>();
    static BigInteger[] depth;
    static BigInteger result = BigInteger.ZERO;

    static void bfs(int start) {
        Queue<Integer> que = new LinkedList<>();
        que.add(start);
        BigInteger count = depth[start] = BigInteger.ONE;

        while (!que.isEmpty()) {
            int node = que.remove();

            for (int i = 0; i < graph.get(node).size(); i++) {
                if (depth[graph.get(node).get(i)] == null) {
                    count = count.add(BigInteger.ONE);
                    que.add(graph.get(node).get(i));
                    depth[graph.get(node).get(i)] = depth[node].add(BigInteger.ONE);
                    result = result.add((depth[graph.get(node).get(i)].subtract(BigInteger.ONE)).multiply(count));
                }
            }
        }
    }
